...
void GPIOA_5_inicializa()
{
	...
	tmpreg = RCC->AHB2ENR;																		//Aguarda estabilizar
...
void GPIOC_13_inicializa()
{
	uint32_t tmpreg = 0;
	//RCC->AHB1ENR |= RCC_AHB1ENR_GPIOCEN; //Habilita o clock do PORTC
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN;
	tmpreg = RCC->AHB2ENR; //Aguarda estabilizar
	GPIOC->MODER = (GPIOC->MODER & ~GPIO_MODER_MODE13_Msk);//M. saída (01)
	//GPIOC->OTYPER = (GPIOC->OTYPER & ~GPIO_OTYPER_IDR_13); //Tipo push-pull (0)
	//GPIOC->OSPEEDR = (GPIOC->OSPEEDR & ~GPIO_OSPEEDR_OSPEED5_Msk) | GPIO_OSPEEDER_OSPEEDR5_1;
	//Velocidade fast (10)
	GPIOC->PUPDR = (GPIOC->PUPDR & ~GPIO_PUPDR_PUPD13_Msk); //Sem resistores internos (00)
	//GPIOA->ODR = (GPIOA->ODR & ~GPIO_ODR_OD5_Msk); //Estado inicial '0'
	//GPIOC->BSRR = GPIO_BSRR_BR5; //Estado inicial resetado '0’
}
/* USER CODE END 4 */
