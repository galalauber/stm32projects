...
BYTE buffer[9600]; /* File copy buffer */
...
//    do
//	{
//      fr = f_read(&fsrc, buffer, sizeof(buffer), (UINT *)&br);
//      if (fr != FR_OK)
//      {
...
//      {
//
//    	  //size = sprintf(texto, "\r\nArquivo lido com sucesso, line = %d br = %d\r\n", line, br);
...
////    	    size = sprintf(texto, "%02X ", buffer[index]);
////    	    HAL_UART_Transmit(&huart2, (uint8_t *)texto, size, 100);
////    	    if (index % 16 == 0) {
////   	    	  size = sprintf(texto, "\r\n");
////    	      HAL_UART_Transmit(&huart2, (uint8_t *)texto, size, 100);
...
////    	  }
//    	  uint16_t *new_buffer;
//    	  new_buffer = (uint16_t *) buffer;
//
//          if (line <= 239) {
//        	drawRGBBitmap(0, line, new_buffer, 320, 15);
//          }
//          line += 15;
//      }
//
...
//  HAL_Delay(1000);
  //invertDisplay(1);
  //fillScreen(BLACK);
  //drawRGBBitmap(0, 239, buffer, 320, 240);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  ...
	  input = HAL_GPIO_ReadPin(B1_GPIO_Port, B1_Pin);
