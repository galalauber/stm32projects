...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define pinA		GPIO_PIN_5	//PB5 - D4 - CLK Enc
#define pinB		GPIO_PIN_4	//PB4 - D5 - DT Enc
#define portA		GPIOB
#define portB		GPIOB
#define minEncVal	0
#define maxEncVal	9
...
GPIO_PinState pinALast;
GPIO_PinState aVal;
int bCW;
uint8_t msgRotacao[] = "Rotacao: ";
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  config_encoder_pins();
  pinALast = HAL_GPIO_ReadPin(portA, pinA);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if ( (aVal != pinALast) && (aVal != 0) ) {
		  ...
		  HAL_UART_Transmit(&huart2, msgVariavel, sizeof(msgVariavel), 50);
	  }
	  pinALast = aVal;
  }
  /* USER CODE END 3 */
